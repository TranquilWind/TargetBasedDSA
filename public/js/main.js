    ...
    document
      ...
      .addEventListener('click', async () => {
        const updated = data.progress.map((p) => ({
          topic: p.topic,
          solved: parseInt(
            document.getElementById(`count-${p.topic}`).textContent
          ),
        }));
        ...
        try {
          ...
          document.getElementById('logs').value = json.logs
            .flatMap((l) => l.entries)
            .join('\n');
      ...
      .addEventListener('click', async () => {
        ...
        try {
          const resp = await fetch('/progress/update-logs', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              logs: [{ date: new Date(), entries: lines }],
            }),
          });
...
async function initDailyLogs() {
  try {
    ...
    document.getElementById('no-logs').textContent =
